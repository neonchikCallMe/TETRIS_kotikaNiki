...
class Tetris:
    ...
    def _get_complete_board(self):
        ...
        for x, y in piece:
            if 0 <= x < Tetris.BOARD_WIDTH and 0 <= y < Tetris.BOARD_HEIGHT:
                board[y][x] = 2 + self.current_piece  # Цвет фигуры начинается с индекса 3
        return board
    ...
    def render(self):
        ...
        img = img[..., ::-1]  # Конвертация RGB в BGR (используется cv2)
        img = Image.fromarray(img, 'RGB')
        img = img.resize((Tetris.BOARD_WIDTH * 25, Tetris.BOARD_HEIGHT * 25), Image.NEAREST)
        img = np.array(img)

        for y in range(1, Tetris.BOARD_HEIGHT):
            cv2.line(img, (0, y * 25), (Tetris.BOARD_WIDTH * 25, y * 25), (0, 0, 0), 1)
        for x in range(1, Tetris.BOARD_WIDTH):
            cv2.line(img, (x * 25, 0), (x * 25, Tetris.BOARD_HEIGHT * 25), (0, 0, 0), 1)

        cv2.putText(img, str(self.score), (24, 24), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
